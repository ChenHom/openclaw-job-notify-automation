...
```bash
python3 bin/application_private_view_server.py --profile-dir /home/hom/services/openclaw-job-notify-profile --host 0.0.0.0 --port 8765
```

- Health endpoint: `http://192.168.50.109/file-exchange/health` from the LAN through the existing nginx port 80 reverse proxy.
- Package endpoint: `http://192.168.50.109/file-exchange/package?applicationId=<applicationId>`.
...
- The package worker stores a private `privateViewUrl` in the local manifest package block, for example `http://192.168.50.109/file-exchange/package?applicationId=<encodedApplicationId>`. This value is private-manifest metadata, not package content in Firestore.
...
- The hosted shell derives the LAN private view URL as `http://192.168.50.109/file-exchange/package?applicationId=<encodedApplicationId>` and does not fetch the private view, auto-redirect, or store private package content in Firestore.
