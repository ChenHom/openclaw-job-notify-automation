...
- Health endpoint: `http://192.168.50.109/health` from the LAN when nginx proxies port 80 to the private view service.
- Package endpoint: `http://192.168.50.109/package?applicationId=<applicationId>`.
...
- The package worker stores a private `privateViewUrl` in the local manifest package block, for example `http://192.168.50.109/package?applicationId=<encodedApplicationId>`. This value is private-manifest metadata, not package content in Firestore.
...
- The hosted shell derives the LAN private view URL as `http://192.168.50.109/package?applicationId=<encodedApplicationId>` and does not fetch the private view, auto-redirect, or store private package content in Firestore.
