...
- The bridge accepts an opaque short-lived local capability token and an `applicationId`.
...
- Do not invent a shared JWT secret in repo code.
- Do not require the local bridge to validate cloud-issued Firebase tokens unless Firebase Admin/JWK verification is explicitly implemented and tested.
- MVP uses local capability tokens generated by the local bridge.
- Store only a hash of the token locally, with `applicationId`, `expiresAt`, and `usedAt`.
- Tokens are single-use or very short TTL.
- Firestore stores only opaque request IDs and non-sensitive status; it does not store local token secrets.
...
- The deterministic validator then performs high-precision checks against structured source data and allowlists.
- The deterministic validator is not a full semantic truth engine. It should only block cases it can prove or strongly indicate from structured evidence.
- It must check:
  - Field allowlist: only `skillsSummaryFull` and `autobiographyFull` may change.
  - Forbidden changes: phone, email, name, education, employment history, dates, salary, title history, and company history.
  - New hard entities: company names, schools, certifications, dates, years of experience, numeric metrics, and contact data not present in source resume or JD.
  - Known alias map: e.g. `AWS` = `Amazon Web Services`, `JS` = `JavaScript`, `GCP` = `Google Cloud Platform`.
- It must not reject safe wording changes solely because exact strings differ.
- Ambiguous cases become `needs_manual_review`, not hard failure.
- Any output that mentions a company, title, technology, metric, tenure, education, certificate, or contact field not present in the source resume, JD, or alias map is rejected or routed to manual review depending on confidence.
...
- Local capability tokens are generated locally, hashed at rest, short-lived, and single-use or near-single-use.
...
- Deterministic validator has alias-map tests, including `AWS` / `Amazon Web Services`, to avoid obvious false positives.
- Deterministic validator routes ambiguous natural-language equivalence to `needs_manual_review` instead of pretending it can prove truth.
